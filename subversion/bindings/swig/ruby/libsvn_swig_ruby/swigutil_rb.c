...
static int
rb_set_pool_for_hash_callback(VALUE key, VALUE value, VALUE arg)
{
  struct rb_set_pool_for_hash_arg *set_pool;
  set_pool = (struct rb_set_pool_for_hash_arg *) arg;
  if (svn_swig_rb_set_pool(value, set_pool->pool))
    set_pool->set = TRUE;
  return ST_CONTINUE;
}
...
static int
svn_swig_rb_to_apr_array_row_prop_callback(VALUE key, VALUE value, VALUE arg)
{
  prop_hash_each_arg_t *prop_arg = (prop_hash_each_arg_t *) arg;
  svn_prop_t *prop;

  prop = apr_array_push(prop_arg->array);
  prop->name = apr_pstrdup(prop_arg->pool, StringValueCStr(key));
...
static int
svn_swig_rb_to_apr_array_prop_callback(VALUE key, VALUE value, VALUE arg)
{
  prop_hash_each_arg_t *prop_arg = (prop_hash_each_arg_t *) arg;
  svn_prop_t *prop;

  prop = apr_palloc(prop_arg->pool, sizeof(svn_prop_t));
  prop->name = apr_pstrdup(prop_arg->pool, StringValueCStr(key));
  prop->value = svn_string_ncreate(RSTRING_PTR(value), RSTRING_LEN(value),
                                   prop_arg->pool);
  APR_ARRAY_PUSH(prop_arg->array, svn_prop_t *) = prop;
  return ST_CONTINUE;
}
...
static int
r2c_hash_i(VALUE key, VALUE value, VALUE data)
{
  hash_to_apr_hash_data_t *data_arg = (hash_to_apr_hash_data_t *) data;
  if (key != Qundef) {
    void *val = data_arg->func(value, data_arg->ctx, data_arg->pool);
    svn_hash_sets(data_arg->apr_hash,
	          apr_pstrdup(data_arg->pool, StringValuePtr(key)), val);
  }
  return ST_CONTINUE;
}
